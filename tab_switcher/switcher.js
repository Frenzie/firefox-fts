...
function scrollToSelection() {
	...
	const tabsContainer = document.getElementById('tabs_list');
	const stringOffset = selectedString.offsetTop;
	const scrollMax = stringOffset - scrollPadding;
	const scrollMin = stringOffset
		+ selectedString.offsetHeight - tabsContainer.offsetHeight + scrollPadding;
	...
	const scrollValue = Math.max(0, scrollMin,
		Math.min(scrollMax, tabsContainer.scrollTop));
	tabsContainer.scrollTop = scrollValue;
}
